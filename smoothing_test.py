        ...
        if current_frame1 <= frame_count1-transition_point:
            ...
            if ret2 == True:
                ...
                if j <= width/2:
                    ...
                    if edge_type == 1 and croppedImage2.shape[1]>=10:
                        frame2[0:0+croppedImage1.shape[0], int(j+width/2)-10:int(j+width/2)+10] = cv2.blur(frame2[0:0+croppedImage1.shape[0], int(j+width/2)-10:int(j+width/2)+10], (20, 20))
                        frame2[0:0+croppedImage2.shape[0], 0+croppedImage2.shape[1]-10:0+croppedImage2.shape[1]+10] = cv2.blur(frame2[0:0+croppedImage2.shape[0], 0+croppedImage2.shape[1]-10:0+croppedImage2.shape[1]+10], (20, 20))
